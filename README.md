...
* Dimmers. Not very useful any more, with low-power lights that doesn't seem to work very well with the dimmer. Can be used as a normal on/off switch at 100% brightness. Appears as lights in Homekit, to be able to set the brightness. Use the `dimmerAsSwitch` configuration to force it to be defined as a normal on/off switch.
