...
//    }
    ...
    this.manageLogLevel(this.tempSensorServices.temperature.characteristicDelegate('logLevel'))
    /*    
    ...
    try {
      ...
      if (response === 'Timeout') {
        ...
        if (response.data[0] && response.data[0].name === 'temp') {
          ...
          this.tempSensorServices.temperature.values.observationTime = observationTime
        }
        else {
