...
class TdSensorAccessory extends homebridgeLib.AccessoryDelegate {
  constructor(platform, params) {
    ...
    this.sensorService.history = new homebridgeLib.ServiceDelegate.History.Weather(
      this, params,
      this.sensorService.characteristicDelegate('temperature')
      //      this.wsServices.humidity.characteristicDelegate('humidity'),
      //      this.wsServices.pressure.characteristicDelegate('pressure')
    )

    this.manageLogLevel(this.sensorService.characteristicDelegate('logLevel'))
    /*    
  ...
  async heartbeat(beat) {
    if (beat % this.sensorService.values.heartrate === 0) {
      this.getSensorData()
      //      this.checkState()
    }
  }
      ...
      if (response === 'Timeout') {
        ...
        if (this.humiditySensor) {
          if (response.data[1] && response.data[1].name === 'humidity') {
            this.sensorService.values.humidity = response.data[1].value
          }
          else {
