...
class TdPlatform extends homebridgeLib.Platform {
  constructor(log, configJson, homebridge) {
    ...
    if (!this.config.ipAddress) {
      ...
      this.log('Found access token for IP:', clc.green(this.config.ipAddress));

      this.tdApi = new TelldusApi(
        this.config.ipAddress,
        this.config.accessToken
      );
      this.log('Telldus API URL:', clc.green(this.tdApi.getUrl));
    }
  ...
  async init(beat) {
    ...
    try {
      let sysInfo = await this.multiApi.getSystemInfo();
      this.log('Telldus system type: ', clc.green(sysInfo.product));
      this.log('Telldus system version:', clc.green(sysInfo.version));
      this.log(
        'Telldus system time:',
        clc.green(isoDateTimeToIntl(sysInfo.time, this.config.locale))
      );
