...
class TdPlatform extends homebridgeLib.Platform {
  ...
  async init(beat) {
    ...
    for (const i in this.devices) {
      this.log('ID:', this.devices[i].id)
      const info = await getDeviceInfo(this.devices[i].id)
      config.id = info.id
      config.name = info.name || 'Device ' + info.id
      ...
      this.log(config.model, config.manufacturer)
    ...
    this.config.switches = validSwitches

    const jobs = []
    for (const tdSwitch of this.config.switches) {
      const switchParams = {
        name: tdSwitch.name,
        id: tdSwitch.uuid,
...
//        useConfig: delaySwitch.useConfig,
      ...
      this.log('Processing', switchParams.name)
