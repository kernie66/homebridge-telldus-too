...
class TdPlatform extends homebridgeLib.Platform {
  constructor(log, configJson, homebridge) {
    ...
    if (!this.config.ipAddress) {
      ...
      this.tdApi = new TelldusApi(this.config.ipAddress, this.config.accessToken)
      this.log('Telldus API URL:', this.tdApi.getUrl);
    }
  ...
  async init(beat) {
    ...
    try {
      let sysInfo = await this.tdApi.getSystemInfo();
      this.log('Telldus system type:', sysInfo.data.product);
      this.log('Telldus system version:', sysInfo.data.version);
      this.log('Telldus system time:', isoDateTimeToIntl(sysInfo.data.time, this.config.locale));
