...
class TdPlatform extends homebridgeLib.Platform {
  ...
  setStateCache(device) {
    ...
    const success = this.stateCache.set(key, device.state);
    if (!success) {
      this.warn(
        "Couldn't cache state for Telldus devices, will try again"
      );
      ...
      this.log();
      'Stored state [%s] for key %s', stateToText(device.state), key;
    }
  }
