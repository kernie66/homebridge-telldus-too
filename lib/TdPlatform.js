...
class TdPlatform extends homebridgeLib.Platform {
  ...
  async init(beat) {
    ...
    for (const id of this.sensorArray) {
      ...
      if (temperatureSensors.includes(info.model)) {
        config.temperatureSensor = true
      }
      if (humiditySensors.includes(info.model)) {
...
//        useConfig: delaySwitch.useConfig,
      ...
      this.log('Processing %s %s, State: [%s]', switchParams.modelType, switchParams.name, switchParams.state)
        ...
        if (this.numberOfDevices) {
          devices.forEach(element => {
            ...
            if (!success) {
              ...
              this.debug('Stored state %s for key %s', element.state, key)
            }
          })
        }
        else {
