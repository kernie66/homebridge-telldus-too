...
const { stateToText, wait, sleep } = require('./utils/utils');
const {
  isoDateTimeToIntl,
  timestampToIntl,
  getTimestamp,
...
const TdTellstick = require('./TdTellstick.js');
...
class TdPlatform extends homebridgeLib.Platform {
  constructor(log, configJson, homebridge) {
    ...
    if (!this.config.ipAddress) {
      ...
      this.tellstick = new TdTellstick(this, {
        config: this.config,
      });
  ...
  async init() {
    ...
    do {
      ...
      try {
        const sysInfo =
          await this.tellstick.telldusApi.getSystemInfo();
        if (!sysInfo.ok) {
    ...
    try {
      ...
      while (retry) {
        deviceResponse =
          await this.tellstick.telldusApi?.listDevices();
      ...
      while (retry) {
        sensorResponse =
          await this.tellstick.telldusApi?.listSensors();
    ...
    for (const id of this.deviceArray) {
      ...
      try {
        const infoResponse =
          await this.tellstick.telldusApi?.getDeviceInfo(id);
        if (!infoResponse.ok) {
    ...
    for (const id of this.sensorArray) {
      ...
      try {
        const infoResponse =
          await this.tellstick.telldusApi?.getSensorInfo(id);
        if (!infoResponse.ok) {
          checkStatusCode(infoResponse, this);
          this.warn(
