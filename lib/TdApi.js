...
async function getDevices(api, timeout = 3000) {
  ...
  .catch( (error) => {
    return('Timeout')
    return('Error getting Telldus devices: ' + error)
  })
  return response
}
...
async function getDeviceInfo(api, id, timeout = 3000) {
  ...
  .catch( (error) => {
    return('Timeout')
    return('Error getting Telldus device info: ' + error)
  })
  return response
}
...
async function setDevice(api, id, state) {
  ...
  .catch( (error) => {
    return('Timeout')
    return('Error setting Telldus device on/off: ' + error)
  })
  return response
}
...
async function dimDevice(api, id, level) {
  ...
  .catch( (error) => {
    return('Timeout')
    return('Error setting Telldus device dim level: ' + error)
  })
}
...
async function getSensors(api) {
  const response = Promise.race([
    ...
    .catch( (error) => {
      return('Timeout')
      return('Error getting Telldus sensors: ' + error)
    })
  return response
}
...
async function getSensorInfo(api, id) {
  ...
  .catch( (error) => {
    return('Timeout')
    return('Error getting Telldus sensor info: ' + error)
  })
  return response
}
