...
class Rain extends RainSensorService {
  ...
  checkObservation(observation) {
    if (observation.data[0] && observation.data[0].name === 'rtot') {
      this.values.rain24h = Math.round(observation.data[0].value * 10) / 10
      let rainRate = Math.round(observation.data[1].value * 10) / 10;
