...
//    this.useConfig = params.useConfig
    ...
    this.stateCache = params.stateCache
...
*/
  ...
  async heartbeat (beat) {
    let beatOffset = beat + this.deviceId
    if (beatOffset % this.switchService.values.heartrate === 0) {
      this.checkState()
      const value = this.stateCache.get('beat')
      if (value === undefined) {
        this.warn('Cached value does not exist')
      }
      else {
        this.log('Cache value =', value)
      }
    }
  }

  async checkState () {
    const result = await getDeviceInfo(this.api, this.deviceId, 10000)
    if (result === 'Timeout') {
      this.warn('Timeout when getting info from device ID', this.deviceId)
    }
    else {
      // Check each correct state to avoid undefined values
      if (result.state === telldus.TURNON) {
        this.switchService.values.on = true
      }
      else if (result.state === telldus.TURNOFF) {
