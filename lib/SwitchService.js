...
class SwitchService extends homebridgeLib.ServiceDelegate {
  ...
  async setOn(switchAccessory) {
    ...
    if (this.delayTimerActive) {
      try {
        this.log('Aborting the delay timer before time is up');
        this.ac.abort();
      } catch (e) {
        this.warn('Error when aborting delay timer: %s', e.message);
      }
      this.delayTimerActive = false;
    }
    if (this.repeatTimerActive) {
      try {
        this.log('Aborting the repeat timer before time is up');
        this.ac.abort();
      } catch (e) {
        this.warn('Error when aborting repeat timer: %s', e.message);
      }
      this.repeatTimerActive = false;
    }
    ...
    do {
      ...
      // Check if thw switch activation shall be repeated
      if (this.values.repeats) {
        ...
        this.repeatTimerActive = true;
        try {
          await wait(3000 + this.values.repetition * 1000, {
            signal: this.signal,
          });
        } catch (err) {
    ...
    } while (this.values.repetition < this.values.repeats);
