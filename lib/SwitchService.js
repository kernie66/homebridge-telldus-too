...
class SwitchService extends homebridgeLib.ServiceDelegate {
  ...
  async setOn(switchAccessory) {
    ...
    do {
      ...
      // If it is a dimmer, then send the current brightness value
      if (this.modelType === 'dimmer') {
        this.setDimmerLevel(switchAccessory, this.values.brightness);
      }
  ...
  async setDimmerLevel(switchAccessory, dimLevel) {
    ...
    */
    ...
    try {
      ...
      //} else {
      this.warn('Error setting dimmer level: %s', err.message);
      //}
      // this.dimming = false;
    }
  }
