...
const getTimestamp = TdTypes.getTimestamp
const wait = TdTypes.wait
const toTime = TdTypes.toTime
const toSeconds = TdTypes.toSeconds
  ...
  constructor(switchAccessory, params = {}) {
    ...
    if (switchAccessory.minDelay) {
      ...
      if (minDelayValue > 863999) {
        ...
        this.warn('Too large min delay time, reset to maximum allowed')
      }
    }
    ...
    this.addCharacteristicDelegate({
      ...
      this.switchOn = value
      this.log("Calling 'setOn' with value %s", this.switchOn)
    ...
    this.addCharacteristicDelegate({
      ...
      this.values.delay = this.values.delayDays * 24 * 3600 + value * 3600 + this.values.delayMinutes * 60 + this.values.delaySeconds
      this.values.minDelay = this.values.delay * this.values.minDelayPercentage / 100
    })
    ...
    this.addCharacteristicDelegate({
      ...
      this.values.delay = this.values.delayDays * 24 * 3600 + this.values.delayHours * 3600 + value * 60 + this.values.delaySeconds
      this.values.minDelay = this.values.delay * this.values.minDelayPercentage / 100
    })
    ...
    this.addCharacteristicDelegate({
      ...
      this.values.delay = this.values.delayDays * 24 * 3600 + this.values.delayHours * 3600 + this.values.delayMinutes * 60 + value
      this.values.minDelay = this.values.delay * this.values.minDelayPercentage / 100
    })
