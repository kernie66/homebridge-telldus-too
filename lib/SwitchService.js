...
class SwitchService extends homebridgeLib.ServiceDelegate {
      ...
      .on('didSet', (value) => {
        ...
        if (!this.values.disabled && !this.values.enabled) {
          ...
          this.log(
            ...
            clc.green('deactivate it to turn it on/off!')
          );
        }
      })
      .on('didTouch', (value) => {
  ...
  async setOn(switchAccessory) {
    ...
    const key = 'piID' + switchAccessory.deviceId;
    const success = switchAccessory.stateCache.set(key, telldusState);
    if (success) {
      this.log(
        'Plug-In state cache set for %s with value [%s]',
        key,
    ...
    // Wait 15 seconds before allowing Telldus to update from cache after switch set
